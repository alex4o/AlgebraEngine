...
Number::Number(int num, Fraction pow)
...
{
	if(this->isNatural()){
		return (expo(2,fraction.up)-2)%fraction.up;
	}
}
...
void Number::print(bool ignoreOne, bool ignoreSign, stringstream& ss)
{
	...
	else if(isRational())
	{
		...
		ss<<"value: "<<fraction.up<<"/"<<fraction.down
		<<" root:"<<pow.down<<"\n";
	}
	if(ignoreSign and negative) fraction.up*=-1;
...
void Number::operator ^=(int k)
{
	fraction.up = expo(fraction.up,k);
	fraction.down = expo(fraction.down,k);
}

Number Number::operator ^(int k)
{
	return Number(expo(fraction.up,k),expo(fraction.down,k));
}
